    ...
    try {
        if (!authorization) {
            return res.
                ...
                send(failResponse("Authentication 1 failed."))
        }
        ...
        if (!token) {
            return res.
                status(parseInt(process.env.AUTHORIZATION_FAIL_CODE)).
                send(failResponse("Authrntication 2 failed."))
        }
        ...
        if (!verify) {
            return res.
                status(parseInt(process.env.AUTHORIZATION_FAIL_CODE)).
                send(failResponse("Authorization 3 failed."))
        }
        ...
        if (!isSuccess) {
            return res.
                status(parseInt(process.env.AUTHORIZATION_FAIL_CODE)).
                send(failResponse("Authorization 4 failed."))
        }
        ...
        return res.
            status(parseInt(process.env.AUTHORIZATION_FAIL_CODE)).
            send(failResponse("Authorization 5 failed."))
    }
 }